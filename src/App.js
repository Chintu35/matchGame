...
class App extends Component {
  ...
  changeScore = id => {
    const {activeImgId, active} = this.state
    ...
    if (activeImgId === id) {
      ...
      const shuffledImageId = imagesList[randomNum].id

      this.setState({activeImgId: shuffledImageId})
    } else {
  ...
  renderGame = () => {
    const {activeImgId, activeTab} = this.state
    const activeImgDetails = imagesList.find(
      eachItem => eachItem.id === activeImgId,
    )
    const activeImg = activeImgDetails ? activeImgDetails.imageUrl : ''

    return (
      <div className="match-game">
        <img className="image-url" src={activeImg} alt="match" />
        ...
        <ul className="thumbnailUrl-items">
  ...
  render() {
    ...
    return (
      <div className="match-game-container">
        ...
        {active ? (
          <div className="game-container">{this.renderGame()}</div>
        ) : (
          <Gameover score={score} gameRestart={this.gameRestart} />
        )}
      </div>
    )
  }
